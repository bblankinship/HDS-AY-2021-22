#### Load the libraries ----
library(tidyverse)
#you will need to install the following packages using install.package("package name") before running the library commands
library(lubridate)
...
mortality %>%
  ...
  theme_minimal() + 
  ggtitle("Heart Disease Deaths in Scotland by Sex") + 
...
mortality %>%
  ...
  ggplot(aes(x = Diagnosis, y = NumberOfDeaths, fill = Sex)) + 
  geom_col(position = "dodge") + 
  ggtitle("Heart Disease Deaths in Scotland in 2014") + 
  ylab("Number of Deaths")
...
mortality %>%
  ...
  geom_col(position = "dodge") + 
  theme_bw() + 
  labs(title = "Heart Disease Deaths", subtitle = "People under 45 in NHS Lothian in 2014") + 
  ylab("Number of Deaths")
